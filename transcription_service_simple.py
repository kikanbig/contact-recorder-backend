#!/usr/bin/env python3
"""
Упрощенный сервис транскрипции через WhisperX без диаризации
Fallback версия для случаев проблем с pyannote.audio
"""
...
def transcribe_audio_simple(audio_data, language='ru', model_size='small'):
    """
    Транскрибирует аудио данные через WhisperX без диаризации
    ...
    """
    try:
        print(f"🚀 Загружаем WhisperX модель (без диаризации): {model_size}", file=sys.stderr)
        ...
        try:
            ...
            print(f"✅ WhisperX транскрипция завершена (без диаризации)", file=sys.stderr)
...
def main():
    ...
    try:
        ...
        # Транскрибируем с WhisperX без диаризации
